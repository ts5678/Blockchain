...
export class PlanningDashboardComponent {
    ...
    constructor(private OSService: OrderSystemService, private web3service: Web3Service, private toastr: ToastrService) {
    ...
    ngOnInit() {
        ...
        this.OSService.orderNotify.asObservable().subscribe({
            next: result => {
                console.log('planning result received ==> ' + result);
                console.log(`Intial data Loaded = ${this.OSService.orderNotifyLoadComplete}`);
                this.rows = [...this.rows];
        ...
        this.GetTransactions().then((value) => { this.intialOrdersLoaded = true; });
    }
